...
UserProvider = function(host, port) {
  this.db= new Db('socketwebapp', new Server(host, port, {safe: false}, {auto_reconnect: true}, {}));
...
// update an employee
UserProvider.prototype.update = function(employeeId, employees, callback) {
    this.getCollection(function(error, employee_collection) {
      if( error ) callback(error);
      else {
...
//delete employee
UserProvider.prototype.delete = function(employeeId, callback) {
	this.getCollection(function(error, employee_collection) {
		if(error) callback(error);
		else {
